...
class LambdaLayer(nn.Module):
    def __init__(
        ...
        self.w_q = nn.Conv2d(dim, dim_k * heads, 1, bias=False)
        self.w_k = nn.Conv2d(dim, dim_k * dim_u, 1, bias=False)
        self.w_v = nn.Conv2d(dim, dim_v * dim_u, 1, bias=False)
    ...
    def forward(self, x):
        ...
        q = self.w_q(x)  # b, dim_k * heads, hh, ww
        k = self.w_k(x)  # b, dim_k * dim_u, hh, ww
        v = self.w_v(x)  # b, dim_v * dim_u, hh, ww
